...
onready var Breaker: PackedScene = load("res://game/level/breakers/Breaker.tscn")
onready var Cracker: PackedScene = preload("res://game/level/breakers/Cracker.tscn")
...
func _spawn_new_breaker(new_braker_polygon: PoolVector2Array, spawn_and_slice: bool = false, is_debry: bool = false):
	...
	if is_debry:
		...
		var centralized_data: Array = operator.centralize_polygon_position(new_braker_polygon)
		new_braker_polygon = centralized_data[0]
...
func _spawn_breaker(new_braker_polygon: PoolVector2Array, spawn_and_slice: bool = false):
...
#	# _temp  nadaljujem v novi metodi, da je lahko deferred
#	breaker_parent.call_deferred("add_child", new_breaker)
#	call_deferred("_set_spawned_breaker", new_breaker, new_braker_polygon, spawn_and_slice)
#func _set_spawned_breaker(spawned_breaker: RigidBody2D, new_breaker_base_polygon: PoolVector2Array, spawn_and_slice: bool):
#
#	spawned_breaker.breaker_base.color = breaker_base.color
#	if breaker_base.texture:
#		_copy_texture_between_shapes(spawned_breaker.breaker_base, breaker_base)
#	# setget
#	spawned_breaker.breaker_base_polygon = new_breaker_base_polygon
#	if spawn_and_slice:
#		spawned_breaker.call_deferred("_slice_chunks", [spawned_breaker.breaker_base_polygon], true)



func _spawn_debry_breaker(debry_polygon: PoolVector2Array):

	# centraliziram in globaliziram
	var centralized_data: Array = operator.centralize_polygon_position(debry_polygon)
	debry_polygon = centralized_data[0]
	var centralized_position: Vector2 = centralized_data[1] # ofsetana pozicija znotraj brejkerja
	var centralized_global_position: Vector2 = centralized_position + position
	...
	# setgets
	new_breaker.crack_width = 2
	new_breaker.breaker_base_polygon = debry_polygon
	new_breaker.current_motion = new_breaker.MOTION.EXPLODE


#func _spawn_debry(debry_polygons: Array):
#
...
#		print("hide_____")
#		polygon_shadow.hide()
#	call_deferred("emit_signal", "shape_changed", breaker_base_polygon)
