...
func _process(delta: float) -> void:
	...
	if is_set and visible:
		...
		if health_bar.visible:
			health_bar_line.rect_scale.x = hud_agent.driver_stats[Pfs.STATS.HEALTH]
		...
		# manage gas bar
		if gas_bar.visible:
			gas_bar_line.rect_scale.x = hud_agent.driver_stats[Pfs.STATS.GAS] / hud_agent.gas_tank_size
		...
		if agent_is_ai:
			...
			for weapon in selector_items.values():
				if "weapon_ammo" in weapon:
					...
					var ammo_count: float = hud_agent.driver_stats[ammo_count_key]
...
func _add_weapon_selector(item_weapon: Node2D):
	...
	if "weapon_ammo" in item_weapon:
		...
		new_selector_item.get_node("CountLabel").text = "%02d" % hud_agent.driver_stats[weapon_ammo_count_key]
	else:
...
func _on_next_weapon_selected(selected_index: int):
	...
	if is_set and not selector_items.empty() and not agent_is_ai:
