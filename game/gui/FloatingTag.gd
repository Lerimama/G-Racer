...
func _physics_process(delta: float) -> void:
	...
	var x_limit_right: float = Rfs.current_camera.get_camera_screen_center().x + get_viewport().size.x/2 - x_offset
	var y_offset: float = 20 # razlika v višini po animaciji
	var y_limit_left: float = Rfs.current_camera.get_camera_screen_center().y - get_viewport().size.y/2 + y_offset
	var y_limit_right: float = Rfs.current_camera.get_camera_screen_center().y + get_viewport().size.y/2 - y_offset
