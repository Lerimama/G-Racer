...
func _motion_machine():
	...
	match motion:
		MOTION.FWD:
			if not is_ai:
				var rotate_to_angle: float = rotation_dir * deg2rad(max_engine_rotation_deg) + bolt.global_rotation
				force_rotation = lerp_angle(force_rotation, rotate_to_angle, engine_rotation_speed)
...
#			else:
##				force_rotation določa AI
			force_on_bolt = Vector2.LEFT.rotated(force_rotation) * bolt.engine_power
