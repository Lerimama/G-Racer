...
#	NO_CONTROLS,
#	}
var current_drive_rotation: int = DRIVE_ROTATION.NONE
export (DRIVE_ROTATION) var selected_drive_rotation: int = DRIVE_ROTATION.MASSLESS

#export (DRIVE_ROTATION) var drive_rotation: int = 0 setget _change_drive_rotation
...
onready var max_engine_power = bolt.bolt_profile["max_engine_power"]
...
var start_params_are_set: bool = false
onready var all_mass: float = bolt.bolt_profile["masa"]
onready var front_mass_bias = bolt.bolt_profile["front_mass_bias"]


func _ready() -> void:
	# bolt seta prvi driving, ker vključuje tudi njegovo nastavljanje
	pass


func _process(delta: float) -> void:

	if not start_params_are_set:
#		self.drive_rotation = drive_rotation
#		_use_drive_rotation(selected_drive_rotation)
		start_params_are_set = true
	...
	# debug
	bolt.bolt_hud.turning_label.text = DRIVE_ROTATION.find_key(current_drive_rotation)
	bolt.bolt_hud.float_rotation_label.text = IDLE_ROTATION.find_key(idle_rotation)
...
func _motion_machine():
	...
	torque_on_bolt = 0
	_use_drive_rotation(DRIVE_ROTATION.NONE)
	_use_drive_rotation(DRIVE_ROTATION.NONE)
	...
	match motion:
		...
		MOTION.REV:
			if rotation_dir == 0:
				...
				_use_drive_rotation(selected_drive_rotation)
			force_rotation = lerp_angle(force_rotation, rotation_dir * deg2rad(max_engine_rotation_deg) * - 1, engine_rotation_speed)
...
#						bolt.force = Vector2.DOWN.rotated(bolt.rotation) * rotation_dir
						...
						for thrust in bolt.engines.all_thrusts:
							thrust.start_fx()
					IDLE_ROTATION.ROTATE:
			#			bolt.angular_damp = 0 # če tega ni moraš prekinit tipko, da se preklopi preko IDLE stanja
...
func _use_drive_rotation(drive_rotation_to_use: int):
	# če ne zavija je fizika celega bolta
	# če zavija se porazdeli glede na stil

	if bolt:

		current_drive_rotation = drive_rotation_to_use
		var mass_summ: float

		match drive_rotation_to_use:
			DRIVE_ROTATION.NONE: # mid zavijanje + malo drifta
				engine_power_adon = 0
				bolt.mass = all_mass
				bolt.angular_damp = 100
				bolt.linear_damp = 1
				bolt.front_mass.linear_damp = 0
				bolt.rear_mass.linear_damp = 0
				bolt.front_mass.mass = 0
				bolt.rear_mass.mass = 0
			DRIVE_ROTATION.MASSLESS: # mid zavijanje + malo drifta
				engine_power_adon = 500
...
#func _change_drive_rotation(new_drive_rotation: int):
#
#	if bolt:
#		drive_rotation = new_drive_rotation
#
#		var mass_summ: float
#		match drive_rotation:
#			DRIVE_ROTATION.NONE: # mid zavijanje + malo drifta
...
#			DRIVE_ROTATION.AGILE: # hitro vijuganje, nežno driftanje
#				max_engine_power = 700 * 2#000
#				front_mass_bias = 0.5
#				bolt.angular_damp = 5
#				bolt.front_mass.linear_damp = 2
#				bolt.rear_mass.linear_damp = 8
#
#			DRIVE_ROTATION.TRACKING: # dolgo zavijanje, no drift
#				engine_rotation_speed = 0.5
#				max_engine_power = 800 * 2#000
#				front_mass_bias = 0.5
...
#
#			DRIVE_ROTATION.DRIFTING: # krajše zavijanje + drift
#				engine_rotation_speed = 0.6
#				max_engine_power = 700
#				front_mass_bias = 0.2
...
#				max_engine_rotation_deg = 35
		...
		# porazdelitev mase
#		if drive_rotation == DRIVE_ROTATION.MASSLESS:
		bolt.front_mass.mass = all_mass * front_mass_bias
		bolt.rear_mass.mass = all_mass * (1 - front_mass_bias)
#		else:
#			bolt.mass = all_mass
#			var front_rear_mass: float = bolt.front_mass.mass + bolt.rear_mass.mass
#			bolt.front_mass.mass = front_rear_mass * front_mass_bias
#			bolt.rear_mass.mass = front_rear_mass * (1 - front_mass_bias)
...
func _zaloga_turningov():
	pass
#		DRIVE_ROTATION.SLOW_VOH: # dolgo zavijanje, no drift
#			bolt.mass = 1
#			engine_rotation_speed = 0.5
#			max_engine_power = 250#800#000
#			front_mass_bias = 0.5
#			bolt.angular_damp = 0
#			bolt.front_mass.linear_damp = 0
...
#			#
#			front_mass_bias = bolt.bolt_profile["front_mass_bias"]
#			bolt.angular_damp = bolt.bolt_profile["ang_damp"]
