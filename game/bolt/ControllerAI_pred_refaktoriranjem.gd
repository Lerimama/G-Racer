...
enum AI_STATE {OFF, RACE, SEARCH, FOLLOW, HUNT, MOUSE_FOLLOW} # ai je možgan (controller in ne bolt
var current_ai_state: int = AI_STATE.OFF # !!! stanja se setajo samo s tarčami
...
func _physics_process(delta: float) -> void:
	...
	# ko je aktiven dela state_mašina (OFF), ne pa tudi pogon
	if controlled_bolt.is_active:
		...
		if Refs.game_manager.game_on:
			if is_processing_input():
				...
				set_ai_target(null) # postane OFF
		else:
			set_ai_target(null)
...
#		else:
			...
			if current_ai_state == AI_STATE.RACE:
				current_target_position = get_racing_position(controlled_bolt.bolt_tracker)
			else:
...
#		set_ai_target(level_navigation_target)
	...
	match current_ai_state:
		...
		AI_STATE.OFF: # target = null ... miruje s prižganim motorjem
...
func set_ai_target(new_ai_target: Node2D):
	...
	# OFF
		...
		#		printt("start OFF from %s" % AI_STATE.keys()[current_ai_state], "new_target: %s" % new_ai_target)
		current_ai_state = AI_STATE.OFF
...
#	current_motion = MotionStates.OFF
	pass
...
##		set_ai_target(controlled_bolt.bolt_tracker.global_position)
#
...
func _on_game_state_change(new_game_state: bool):
...
func _on_message_from_game_manager(message_content):
