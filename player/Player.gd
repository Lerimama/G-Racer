...
export (float, 0, 1) var bounce_size = 0.5 # velikost odboja		
...
var fwd_motion: bool = false # za ločitev ali gre mašina v rikverc ali naprej
...
# import
onready var BoltSprite: Sprite = $Bolt # samo za korekcijo kota ob spawnanju
...
func input_motion(delta: float) -> void:
	...
	if Input.is_action_pressed("up") && motion_enabled == true:
	...
	elif Input.is_action_just_released("up") && motion_enabled == true:
		fwd_motion = false
		engine_particles_rear.set_emitting(false)
			
	if Input.is_action_pressed("down") && motion_enabled == true:
	...
	elif Input.is_action_just_released("down") && motion_enabled == true:
		rev_motion = false
	...
	# prosto vrtenje
	if Input.is_action_pressed("down") == false && Input.is_action_pressed("up") == false && motion_enabled == true: # ko ni gasa niti bremze
...
#		velocity -= velocity/0.7 # ...naj se kar ustavi, da ne bo neskončno računal pozicije
		...
		new_blast.position.x = GunPosition.global_position.x - 10
		new_blast.position.y = GunPosition.global_position.y
		new_blast.rotation = global_rotation
		new_blast.spawned_by = name # ime avtorja izstrelka
