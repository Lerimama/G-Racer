...
# breaker shape2D
export (NodePath) var breaker_base_path: String
export (NodePath) var breaker_collision_path: String
export (int) var crack_width: float = 0 setget _change_crack_width
var breaker_base_polygon: PoolVector2Array
onready var breaker_spawn_parent: Node = get_parent()
onready var debry_spawn_parent: Node = owner.get_parent()

export var is_breakable: bool = true
onready var breaker_crack: Polygon2D = $BreakerCrack
...
func _ready() -> void:
	...
	# če ima vezan node, ga prevzame
	if not breaker_base_path.empty():
		self.breaker_base_polygon = get_node(breaker_base_path).polygon
	# če ni vezanega nodeta in ni podan spawn poly
	elif breaker_base_polygon.empty():
		self.breaker_base_polygon = polygon
	# če je podan spawn poly
	else:
...
var new_brejker_shape_rename: Polygon2D


#func on_hit_resource(hitter: Node2D, hit_global_position: Vector2, hit_target_shape: Polygon2D): # shape je lahko: polygon2D, coližn shape poly, ... če se kaj pojavi vneseš tukaj
#
#	new_brejker_shape_rename = hit_target_shape
#	breaker_base_polygon
...
func split_chunk_to_polygons(chunk_polygon: PoolVector2Array):
	...
	match origin_location_on_shape:
		-1: # zunaj
			...
			sliced_chunk_polygons = operator.split_delaunay(chunk_polygon, -1, 10)
		0:
			...
			sliced_chunk_polygons = operator.split_daisy(chunk_polygon, origin_point_index)[0]
		1:
...
func spawn_breaker(new_braker_polygon: PoolVector2Array, spawn_and_slice: bool = false):
	...
	breaker_spawn_parent.call_deferred("add_child", new_breaker)
...
func set_spawned_breaker(spawned_breaker: RigidBody2D, new_breaker_base_polygon: PoolVector2Array, spawn_and_slice: bool):
	...
	spawned_breaker.breaker_base.color = color
	if texture:
		copy_texture_between_shapes(spawned_breaker.breaker_base, self)
	# setget
...
func spawn_debry(debry_polygons: Array):
	...
	for poly in debry_polygons:
		...
		var debry_breaker = Breaker.instance()
		debry_breaker.name = "Breaker_Debry"
		debry_breaker.position = centralized_global_position
		debry_breaker.is_breakable = false
		debry_spawn_parent.add_child(debry_breaker)

		if texture:
			copy_texture_between_shapes(debry_breaker.breaker_base, self)
			debry_breaker.breaker_base.texture_offset = debry_breaker.position - position
		debry_breaker.breaker_base.color = color
		debry_breaker.break_origin_global = break_origin_global # za animacijo

		# setgets
		debry_breaker.crack_width = 2
		debry_breaker.breaker_base_polygon = poly
		debry_breaker.current_motion = debry_breaker.MOTION.EXPLODE
...
func spawn_chunk(new_chunk_polygon: PoolVector2Array):
	...
	if texture:
		copy_texture_between_shapes(new_poly, self)
...
func spawn_crackers(cracked_polygons: Array, chunk_polygon: PoolVector2Array):
	...
	for poly_index in cracked_polygons.size():
		...
		new_cracker.cracker_color = color
		new_cracker.crack_color = Color.black
		crackers_parent.add_child(new_cracker)

		if texture:
			copy_texture_between_shapes(new_cracker, self)
...
func _change_breaker_polygon(new_breaker_polygon: PoolVector2Array):
	...
	self.crack_width = crack_width
	get_node(breaker_collision_path).set_deferred("polygon", breaker_base_polygon)
