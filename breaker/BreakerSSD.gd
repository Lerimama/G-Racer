...
func _update_ssd_shape():
	# zgradim resource points dictionary

	var new_ssd_points_props: Dictionary = {}
	var new_ssd_points_order: Array = []

	for point_index in $CollisionPolygon2D.polygon.size():

		# props ... kopiram prvega in potem vse  trenutne prebrišem
		var point_props: Resource = $SS2D_Shape_Closed._points._points[0].duplicate()
		point_props.position = $CollisionPolygon2D.polygon[point_index] # point_in / out ne rabim, ker je iz poligona
		new_ssd_points_props[point_index] = point_props

		# order ... index v poligonu je zaporedna številka
		new_ssd_points_order.append(point_index)

	# add next key ... po pregledu original ssd je to 2. točka z obrnjenim x/y
	var next_point_key: int = $CollisionPolygon2D.polygon.size()
	var next_point_props: Resource = $SS2D_Shape_Closed._points._points[0].duplicate()
	next_point_props.position = Vector2(new_ssd_points_props[1].position.y, new_ssd_points_props[1].position.x)
	new_ssd_points_props[next_point_key] = next_point_props
	# order ... umestim ga na predzanji
	new_ssd_points_order.insert(next_point_key, next_point_key)
#	new_ssd_points_order.insert(next_point_key, next_point_key)



	var new_ssd_points: Resource = $SS2D_Shape_Closed._points.duplicate()
	new_ssd_points._points = new_ssd_points_props
#	new_ssd_points._next_key = new_ssd_points._points.keys().size() - 1
	new_ssd_points._next_key = next_point_key
	new_ssd_points._point_order = new_ssd_points_order
	$SS2D_Shape_Closed._points = new_ssd_points
#	$SS2D_Shape_Closed.set_deferred("_points", new_ssd_points)
#	$SS2D_Shape_Closed.set_deferred("_points", new_ssd_points)
	# kar je bilo spremenjeno spremenim v kopiranem points resourcu in ga apliciram
#		point_resource_dict.1
#		point_resource_dict["position"] = $CollisionPolygon2D.polygon[point_index]
#		new_ssd_points[point_index] = point_resource_dict
#	printt ("new_ssd_points", new_ssd_points)
#	printt ("old_ssd_points", $SS2D_Shape_Closed._points._points)
#	printt ("new_ssd_points", new_ssd_points[0].position)


func _ready() -> void:

	for point in $SS2D_Shape_Closed._points._points:
		printt ("breaker_shape points", point)
	printt ("point", $SS2D_Shape_Closed._points._points[0])
	printt ("point", $SS2D_Shape_Closed._points._point_order)
	_update_ssd_shape()
...
#			pass
	...
	#	Mets.spawn_indikator_line(intersection_vector_start + position, intersection_vector_end + position, Color.blanchedalmond, get_parent())
