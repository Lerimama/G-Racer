...
export var active_players_count: int = 4 setget _on_change_players
var viewport_containers_in_game: Array
var game_view_index = 0
onready var player_viewport_container = preload("res://scenes/GameView.tscn")
...
func _process(delta: float) -> void:
	$GameView/Viewport/Arena/DKamera.position = $GameView/Viewport/Arena/Player.global_position
	$GameView2/Viewport/Arena/DKamera.position = $GameView/Viewport/Arena/Player2.global_position
...
func _on_change_players(new_player_count):
	...
	match new_player_count:
		...
		3: 
			free_viewports()
			columns = 2
			spawn_viewport(1, vp_scale_3, Global.ppp1)
			spawn_viewport(2, vp_scale_3, Global.ppp2)
