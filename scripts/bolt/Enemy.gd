...
var hit_disabled_time: float = 1.5
...
func shocker_check():
	...
	if vision_ray_left.is_colliding() and vision_ray_right.is_colliding():
		...
		if collider_left.is_in_group(Config.group_arena) and collider_right.is_in_group(Config.group_arena) and not shocker_released:
...
#			# push
			...
			stop_tween.tween_property(self, "velocity", Vector2.ZERO, hit_disabled_time) # tajmiram pojemek 
			yield(get_tree().create_timer(hit_disabled_time), "timeout")
			# enable controls
		...
		# misile	
			...
			disabled_tween.tween_property(self, "velocity", Vector2.ZERO, hit_disabled_time) # tajmiram pojemek 
			yield(get_tree().create_timer(hit_disabled_time), "timeout")
			# enable controls
