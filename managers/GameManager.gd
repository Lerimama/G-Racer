...
#		if spawned_position_index == 0:
	...
	# _temp
	Ref.current_level.racing_track.set_new_bolt_follower(new_bolt)
...
func get_bolts_ranking():
	...
	# najprej sortirami po poziciji followerja, 
	# potem naredim array boltov v istem zaporedju
	# potem razporedim array boltov glede na kroge
	var all_bolts_ranked: Array = []
	var all_bolt_followers: Array = Ref.current_level.racing_track.get_children()
	all_bolt_followers.sort_custom(self, "sort_followers_by_offset")
	for bolt_follower in all_bolt_followers:
		all_bolts_ranked.append(bolt_follower.follower_target)
...
#		printt ("RANKED", current_bolt_rank, ranked_bolt.bolt_id)
...
#		Ref.current_camera.follow_target = level_goal_position
	...
	# dobim offset njegovega followerj
	# potem premaknem taget za neko distanco naprej po krivulji 
	for ranked_enemy in ranked_enemies:
		
		# poiščem pripadajočeka followerja
		var current_bolt_follower: PathFollow2D
		for bolt_follower in all_bolt_followers:
			if bolt_follower.follower_target == ranked_enemy:
				current_bolt_follower = bolt_follower
				break
		# lociram target točko na krivulji
		var enemy_target_point_on_curve: Vector2
		var enemy_target_prediction: float = 50
		var enemy_target_total_offset: float = current_bolt_follower.offset + enemy_target_prediction
		var bolt_follower_curve: Curve2D = current_bolt_follower.get_parent().get_curve()
		enemy_target_point_on_curve = bolt_follower_curve.interpolate_baked(enemy_target_total_offset)
				
#				Ref.current_level.interpolate_baked
#		var enemy_racing_line_offset_point: Vector2 = level_racing_lines[enemy_racing_line_index].get_points()[enemy_on_racing_line[1] + enemy_racing_point_offset]
		printt("point", ranked_enemy.name, enemy_target_point_on_curve)
...
func sort_bolts_by_laps(bolt_on_racing_line_1, bolt_on_racing_line_2): # ascending ... večji index je boljši
	...
	if bolt_on_racing_line_1.laps_finished_count > bolt_on_racing_line_2.laps_finished_count:
	    return true
	return false


func sort_followers_by_offset(first_bolt_follower, second_bolt_follower): # ascending ... večji index je boljši
	
	if first_bolt_follower.offset > second_bolt_follower.offset:
	    return true
	return false
