...
# player old
onready var input_power:Vector2  # moč pritiska (0 ali 1, jp analogna
...
#	acceleration = input_power.x * engine_power
...
#	velocity += acceleration * delta
	# new
	velocity += Vector2(input_power.x * engine_power * delta, 0).rotated(rotation) # zadnja rotacija je dodana, da hitrost spreminja v smeri igralčeve osi
...
func motion_input(delta: float) -> void:
	...
	input_power.x = Input.get_action_strength("forward") - Input.get_action_strength("reverse")
